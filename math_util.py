...
def nearestLine(point_x, point_y, line_start_x, line_start_y, line_end_x, line_end_y):
...
def nearestBlock(point_x, point_y, block_start_x, block_start_y, block_end_x, block_end_y):
    pos = []

    pos.append(nearestLine(point_x, point_y, block_start_x, block_start_y, block_end_x, block_start_y))
    pos.append(nearestLine(point_x, point_y, block_end_x, block_start_y, block_end_x, block_end_y))
    pos.append(nearestLine(point_x, point_y, block_start_x, block_end_y, block_end_x, block_end_y))
    pos.append(nearestLine(point_x, point_y, block_start_x, block_start_y, block_start_x, block_end_y))
...
def nearestArrow(point_x, point_y, way_pos):
    if way_pos == []:
        return [None, None]
    ...
    for i in range(len(way_pos) - 1):
        pos.append(nearestLine(point_x, point_y, way_pos[i].x(), way_pos[i].y(), way_pos[i + 1].x(), way_pos[i + 1].y()))
